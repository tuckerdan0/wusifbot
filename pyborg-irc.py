...
#	        self.channels = IRCDict()
	...
	def on_msg(self, c, e):
		...
		replyrate = ((self.settings.speaking * self.settings.reply_chance)+random.randint(-2, 8))

		# double reply chance if the text contains our nickname :-)
		if body.lower().find(self.settings.myname.lower()) != -1:
			replyrate = 98##replyrate * 2
	...
	def irc_commands(self, body, source, target, c, e):
		...
		if command_list[0] == "!scouter" and len(command_list)==1:
			msg = "!lvl it's over 9000!!!!! Kreygasm"
		if command_list[0] == "!1v1" and command_list[1] == "woosif":
			time.sleep(3)
			msg = "!1v1 " + source
